# -*- coding: utf-8 -*-
"""
FileName:  LakeshoreApp-U8-Flow.py 447 419 071
...
def login_clicked():
    ...
    OutFile = 'C:\\Users\\User Acess\\Desktop\\Test Lab Files Sync\\' + FileName + '.csv'    
...
NoteFileName = 'C:\\Users\\User Acess\\Desktop\\Test Lab Files Sync\\' + FileName + '_Notes.txt'
...
def Read_336():
    ...
    tD1 = round(float(my_model_336.query('KRDG? D1')),3)
    tD2 = round(float(my_model_336.query('KRDG? D2')),3)
    tD3 = round(float(my_model_336.query('KRDG? D3')),3)
    tD4 = round(float(my_model_336.query('KRDG? D4')),3)
    tD5 = round(float(my_model_336.query('KRDG? D5')),3)
...
x_data = []
y_data = [[] for _ in range(9)]  # Create lists for each line
...
lines = []
colors = ['#00A5D1', '#990000', '#009900', '#006666', '#964B00','#FF99FF', '#BFAA98', '#FF3333', '#660099']
labels = ['Sensor A', 'Sensor B', 'Sensor C', 'Sensor D1', 'Sensor D2', 'Sensor D3', 'Sensor D4', 'Sensor D5', 'Flow']
...
def save_data_to_csv(event=None):  # Accept an optional event argument
    with open(csv_filename, mode='w', newline='') as file:
        ...
        for i in range(len(x_data)):
            row = [x_data[i]] + [y_data[j][i] for j in range(9)]
            writer.writerow(row)
