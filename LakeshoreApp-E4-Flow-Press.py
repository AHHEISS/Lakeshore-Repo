# -*- coding: utf-8 -*-
"""
FileName:  LakeshoreApp-E4-Flow-Press.py 447 419 071
...
global comPort,  flowPort, pressPort, LED
...
# comPort
comPort_label = ttk.Label(signin, text="Enter comPorts for Flow, Pressure:")
...
x_data = []
y_data = [[] for _ in range(6)]  # Create lists for each line
...
lines = []
colors = ['#00A5D1', '#990000', '#009900', '#006666', '#66FF33', '#660099']
labels = ['Sensor A', 'Sensor B', 'Sensor C', 'Sensor D', 'Pressure', 'Flow']
...
def save_data_to_csv(event=None):  # Accept an optional event argument
    with open(csv_filename, mode='w', newline='') as file:
        ...
        for i in range(len(x_data)):
            row = [x_data[i]] + [y_data[j][i] for j in range(6)]
            writer.writerow(row)
