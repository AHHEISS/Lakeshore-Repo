# -*- coding: utf-8 -*-
"""
FileName:  LakeshoreApp-U2-Flow.py 447 419 071
...
from lakeshore import model_335
...
my_model_335 = model_335.Model335(57600)
...
def Read_335():
    ...
    tA = round(float(my_model_335.query('KRDG? A')),3)
    tB = round(float(my_model_335.query('KRDG? B')),3)
...
x_data = []
y_data = [[] for _ in range(3)]  # Create lists for each line
...
lines = []
colors = ['#00A5D1', '#990000', '#660099']
labels = ['Sensor A', 'Sensor B', 'Flow']
...
def save_data_to_csv(event=None):  # Accept an optional event argument
    with open(csv_filename, mode='w', newline='') as file:
        ...
        for i in range(len(x_data)):
            row = [x_data[i]] + [y_data[j][i] for j in range(3)]
            writer.writerow(row)
