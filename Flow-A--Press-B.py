# -*- coding: utf-8 -*-
"""
FileName:  Flow-A--Press-B-comPort.py 447 419 071
...
Flow and Pressure mapped to comPort input A and B
...
# comPort
comPort_label = ttk.Label(signin, text="Enter comPort:")
...
#    print('saved_line = ', saved_line)
    ...
    fB = round(float(Flow.split(",")[2]) * 200.0 / 1024.0, 1)
    return(fA, fB)
...
ax2 = ax.twinx()
ax2.set_ylim(0, 200)  # Adjust the y-limits for PressB

# Add a second ylabel for Line 5
ax2.set_ylabel('Pressure [psi]', color='#FF0000')  # Purple


# Create lines for the new function 
FlowA, = ax.plot([], [], lw=2, label='Flow A', color='#660099')  # Purple
PressB, = ax2.plot([], [], lw=2, label='Press B', color='#FF0000')  # Red
